{{-- If you look to others for fulfillment, you will never truly be fulfilled. --}}
<div class="flex flex-row content-between bg-white p-5 rounded shadow-xl" style="width: 18rem">
        <span class="icon bg-green-100 rounded-full w-12 flex align-middle content-center" style="align-items: center"><ion-icon
                style="font-size: 25px" class="flex content-center align-middle mx-auto" name="close-circle-outline"></ion-icon></span>
    <div class="sales-number flex flex-col ml-auto text-center">
        <span class="number text-black font-bold">{{ $unVerifiedUsers }}</span>
        <span class="total-sales font-light opacity-50">Email Unverified Users</span>
    </div>
</div>
